...
class AttachmentApiController with Helpers {
  ...
  Future<AttachmentType> getAttachmentType({required int? id}) async {
    var url = Uri.parse(
        ApiSettings.attachmentTypes.replaceFirst('{id}', id.toString()));
    var response = await http
        .get(url, headers: {HttpHeaders.acceptHeader: 'application/json'});
    var responseJson = jsonDecode(response.body);
    if (response.statusCode == 200) {
      var attachmentType =
          AttachmentType.fromJson(responseJson['attachment_type']);
  ...
  Future<void> createAttachment({
    ...
    http.MultipartFile imageFile =
        await http.MultipartFile.fromPath('url', path);
    ...
    request.fields['attachment_type_id'] =
        attachment.attachmentTypeId.toString();
    ...
    response.stream.transform(utf8.decoder).listen((event) {
      print('response.statusCode for attachment is : ${response.statusCode}');
  ...
  Future<bool> deleteAttachment(
    ...
    var url =
        Uri.parse(ApiSettings.attachments.replaceFirst('{id}', id.toString()));
